...
const SimulateStrategyBacktestOutputSchema = z.object({
  ...
  simulationNarrative: z.string().describe('A detailed narrative describing a plausible historical price movement for the coin over the period and how the given strategy (entry, targets, stop-loss) would have hypothetically performed against this imagined scenario. Mention key inflection points.'),
  keyEvents: z.array(z.string()).optional().describe('A list of key simulated events, e.g., "Entry triggered near simulated day X at $Y", "First target hit on simulated day Z", "Stop-loss would have been activated due to simulated drop".'),
...
6.  **List Key Simulated Events (Optional)**: Bullet point key moments in your simulation (e.g., "Entry at $X on simulated day Y", "Target 1 hit", "Stop-loss triggered").
