...
const InvestmentStrategySchema = z.object({
  ...
  actionableSteps: z.array(z.string()).min(2).max(4).describe('A few (2-4) concrete, actionable steps for implementing this strategy, including setting precise buy orders at the optimal price and sell orders/alerts at target prices. Include key advanced indicators to monitor for entry/exit confirmation (e.g., "Set limit buy at $X, with secondary entry at $Y (0.618 Fib retracement).", "Place tiered take-profit orders: 30% at $T1, 50% at $T2, hold 20% for $T3.", "Monitor Order Flow Delta and Cumulative Volume Delta (CVD) for buy pressure confirmation before entry, and look for divergences at exit targets.").'),
  stopLossSuggestion: z.string().optional().describe('A concrete suggestion for a stop-loss level or sophisticated dynamic strategy (e.g., "Set a hard stop-loss at $X.XX, representing Y% below optimal buy and below key support.", "Implement a trailing stop-loss of Z% based on 20-period ATR once price surpasses first target.", "Use a Chandelier Exit based on a 22-period ATR with a multiplier of 3 for capturing extended moves."). Justify the stop-loss approach.'),
...
2.  **Investment Strategies (Profit Maximization Focus - for Advanced Users)**: Recommend 1 to 3 distinct, advanced investment strategies designed to maximize profit for {{{coinName}}}, considering its profile and the user's parameters. For each strategy:
    ...
    *   **Actionable Steps (2-4 steps)**: Provide concrete, actionable steps for an advanced trader. Examples: "Set a limit buy order at \${{{optimalBuyPrice}}} with a secondary, smaller entry point at \${lower_support_if_applicable} if a wick occurs.", "Place tiered take-profit orders: 30% at \${target_1}, 50% at \${target_2}, hold 20% for \${target_3}.", "Monitor [specific advanced indicator like Cumulative Volume Delta (CVD) or Open Interest] for confirmation of buy pressure before entry and look for signs of distribution (e.g., bearish divergence on CVD) at exit targets."
    *   **Stop-Loss Suggestion**: Provide a concrete stop-loss price or a sophisticated dynamic strategy. Justify it (e.g., "Set initial stop-loss at \${key_support_level_X}, approximately Y% below optimal buy, which invalidates the immediate bullish structure.", "Implement a trailing stop-loss of Z% using the 20-period Average True Range (ATR) once the first profit target is hit to protect gains while allowing for further upside.", "Use a Chandelier Exit based on a 22-period ATR with a multiplier of 3 to ride the trend.").
...
Ensure actionable steps are specific and give indicator examples.
Ensure stop-loss suggestions are concrete and justified.
